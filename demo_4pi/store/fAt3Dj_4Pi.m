function pd = fAt3Dj_4Pi(xc, yc, zc,xsize,ysize,zsize,delta_f,PSF, phi, phi0)
% xc = single(xc);
% yc = single(yc);
% zc = single(zc);
...
temp = PSF.Ispline(xc+1,yc+1,zc,:);
I = sum(delta_f.*(temp(:)));
temp = PSF.Aspline(xc+1,yc+1,zc,:);
A = sum(delta_f.*(temp(:)));
temp = PSF.Bspline(xc+1,yc+1,zc,:);
B = sum(delta_f.*(temp(:)));
