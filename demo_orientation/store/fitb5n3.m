...
initP=single([25 24 25 0.7;25 12 25 0.7;]);
...
silent=1;
[P5, CRLB5, LL]=GPUmleFit_LM_multiD(single(tmpPSF),single(8),iter,...
    single(coeff5),(0),0,single(inittmp),single(coeff3));
...
if size(initP,1)>1
    for i=2:size(initP,1)
        ...
        [P5h,CRLB5h,LLh]=GPUmleFit_LM_multiD(single(tmpPSF),single(8),iter,...
            single(coeff5),(0),silent,single(inittmp),single(coeff3));
