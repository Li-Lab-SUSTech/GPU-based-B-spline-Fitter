function [P,update] =  kernel_MLEfit_Spline_LM_v8_single(d_data,b3,sz,iterations)
...
dataSize = b3.dataSize;
b3.coeffs(end+1,end+1,end+1)=0;
PSFSigma = single(1.5);
xc = single(0);
yc = single(0);
zc = single(0);
...
%     [delta_f,delta_dfx,delta_dfy,delta_dfz] = computeDelta3D_bSpline_v2_single(0.5,0.5,0.5);
    ...
    for ii = 0:sz-1
        for jj = 0:sz-1
            ...
            %             [newDudt,model] = kernel_Derivative_bSpline(xstart+jj+off,ystart+ii+off,zstart,dataSize(1),dataSize(2),dataSize(3),b3.coeffs,delta_f,delta_dfx,delta_dfy,delta_dfz,newTheta);
            [newDudt,model] = kernel_Derivative_bSpline_single(xc+jj+off,yc+ii+off,zc,dataSize(1),dataSize(2),dataSize(3),b3.coeffs,delta_f,delta_dfx,delta_dfy,delta_dfz,newTheta);
            
            %             [temp,dfx,dfy,dfz] = fAt3D_bs_v6(xstart+jj+off,ystart+ii+off,zstart,dataSize(1),dataSize(2),dataSize(3),b3.coeffs,delta_f,delta_dfx,delta_dfy,delta_dfz);
            %             model = single(newTheta(4)+newTheta(3)*temp);
