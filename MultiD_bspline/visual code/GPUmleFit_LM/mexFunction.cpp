...
*  \fitmode 5 cspline
*  \fitmode 6 bspline
*  \fitmode 7 4D bspline
*  \fitmode 8 5D bspline
*  \optional:
...
*  \varargin7. initZ for cspline and bspline fit, default=spline_zsize/2 (focus)
*  \varargin7. initP for 4D bspline fit, default=spline_zsize/2 (focus)
*  \varargin7. initP for 5D bspline fit, default=spline_size/2 (focus)
...
*  \fitmode 6. X, Y, Photons, Background, Z, Iterations
*  \fitmode 7. X, Y, Photons, Background, Z, fourth_variable, Iterations
*  \fitmode 8. X, Y, Photons, Background, Z, fourth_variable, fifth_variable, Iterations
...
extern void kernel_bsplineMLEFit_z_EMCCD_wrapper(dim3 dimGrid, dim3 dimBlock, const float* d_data, const float* d_coeff, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int sz, const int iterations,
	float* d_Parameters, float* d_CRLBs, float* d_LogLikelihood, const int Nfits, float* deg, float* temp);

extern void kernel_bsplineMLEFit_4D_EMCCD_wrapper(dim3 dimGrid, dim3 dimBlock, const float* d_data, const float* d_coeff, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int spline_wsize, const int sz, const int iterations, const float* d_initP4,
	float* d_Parameters, float* d_CRLBs, float* d_LogLikelihood, const int Nfits, float* deg);

extern void kernel_splineMLEFit_5n3D_EMCCD_wrapper(dim3 dimGrid, dim3 dimBlock, const float* d_data, const float* d_coeff5, const float* d_coeff3, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int spline_wsize, const int spline_usize, const int sz, const int iterations, const float* initP5,
	float* d_Parameters, float* d_CRLBs, float* d_LogLikelihood, const int Nfits, float* deg);
...
void cudaavailable(int silent) {
	...
	if (cudaSuccess == cudaRuntimeGetVersion(&runtimeVersion)) {
		if  (silent==0)
			mexPrintf("CUDA runtime version: %d\n", runtimeVersion);
...
void mexFunction(int nlhs, mxArray *plhs[],	int	nrhs, const	mxArray	*prhs[]) {
	...
	*/
	...
	float *d_Parameters=0,*d_CRLBs=0,*d_LogLikelihood=0, * temp = 0;
	...
	float* d_Deg = 0;
	//Spline
	int spline_xsize, spline_ysize, spline_zsize, spline_wsize, spline_usize;
	float *coeff=0,*d_coeff=0, * coeff3 = 0, * d_coeff3 = 0,* initP=0, * d_initP = 0, * d_initP5=0, * d_initP4=0;
	...
	else
	{
		...
		if (nrhs>3){
			...
			case 7:
				...
				spline_ysize = datasize_spline[1];
				spline_zsize = datasize_spline[2];
				spline_wsize = datasize_spline[3];
				break;
			case 8:
				iterations = (int)mxGetScalar(prhs[2]);
				if (mxGetClassID(prhs[3]) != mxSINGLE_CLASS)
				...
				spline_zsize = datasize_spline[2];
				spline_wsize = datasize_spline[3];
				spline_usize = datasize_spline[4];
				break;
...
if  (silent==0)
	...
	case 4:
		...
		break;
	case 5:
		requiredMemory*=NV_PS+1;
		requiredMemory +=spline_xsize*spline_ysize*spline_zsize*64*sizeof(float);
		break;

	case 6:
		requiredMemory *= NV_PS+1;
		requiredMemory += spline_xsize * spline_ysize * spline_zsize * sizeof(float);
		break;


	case 7:
		requiredMemory *= NV_PSP4D + 1;
		requiredMemory += spline_xsize * spline_ysize * spline_zsize * spline_wsize * sizeof(float);
		break;

	case 8:
		requiredMemory *= NV_P53 + 1;
		requiredMemory += spline_xsize * spline_ysize * spline_zsize * spline_wsize* spline_usize * sizeof(float);
		requiredMemory += spline_xsize * spline_ysize * spline_zsize * sizeof(float);
...
if  (silent==0)
{
	...
	if (fittype == 6) {
		...
		cudasafe(cudaMalloc((void**)& d_coeff, spline_xsize * spline_ysize * spline_zsize  * sizeof(float)), "Failed cudaMalloc on d_coeff.", __LINE__);
		cudasafe(cudaMemset(d_coeff, 0, spline_xsize * spline_ysize * spline_zsize *  sizeof(float)), "Failed cudaMemset on d_coeff.", __LINE__);
		cudasafe(cudaMemcpy(d_coeff, coeff, spline_xsize * spline_ysize * spline_zsize  * sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_coeff.", __LINE__);
	}

	if (fittype == 7) {
		cudasafe(cudaMalloc((void**)& d_coeff, spline_xsize * spline_ysize * spline_zsize*spline_wsize * sizeof(float)), "Failed cudaMalloc on d_coeff.", __LINE__);
		cudasafe(cudaMemset(d_coeff, 0, spline_xsize * spline_ysize * spline_zsize* spline_wsize * sizeof(float)), "Failed cudaMemset on d_coeff.", __LINE__);
		cudasafe(cudaMemcpy(d_coeff, coeff, spline_xsize * spline_ysize * spline_zsize* spline_wsize * sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_coeff.", __LINE__);

		cudasafe(cudaMalloc((void**)& d_initP4, 2 * sizeof(float)), "Failed cudaMalloc on d_initP5.", __LINE__);
		cudasafe(cudaMemset(d_initP4, 0, 2 * sizeof(float)), "Failed cudaMemset on d_initP5.", __LINE__);
		cudasafe(cudaMemcpy(d_initP4, initP, 2 * sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_initP5.", __LINE__);


	}
	if (fittype == 8) {
		
		cudasafe(cudaMalloc((void**)& d_coeff, spline_xsize * spline_ysize * spline_zsize * spline_wsize * spline_usize* sizeof(float)), "Failed cudaMalloc on d_coeff.", __LINE__);
		cudasafe(cudaMemset(d_coeff, 0, spline_xsize * spline_ysize * spline_zsize * spline_wsize * spline_usize * sizeof(float)), "Failed cudaMemset on d_coeff.", __LINE__);
		cudasafe(cudaMemcpy(d_coeff, coeff, spline_xsize * spline_ysize * spline_zsize * spline_wsize * spline_usize * sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_coeff.", __LINE__);
		
		cudasafe(cudaMalloc((void**)& d_coeff3, spline_xsize* spline_ysize* spline_zsize * sizeof(float)), "Failed cudaMalloc on d_coeff.", __LINE__);
		cudasafe(cudaMemset(d_coeff3, 0, spline_xsize* spline_ysize* spline_zsize * sizeof(float)), "Failed cudaMemset on d_coeff.", __LINE__);
		cudasafe(cudaMemcpy(d_coeff3, coeff3, spline_xsize* spline_ysize* spline_zsize* sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_coeff.", __LINE__);

		cudasafe(cudaMalloc((void**)& d_initP5, 4 * sizeof(float)), "Failed cudaMalloc on d_initP5.", __LINE__);
		cudasafe(cudaMemset(d_initP5, 0, 4 * sizeof(float)), "Failed cudaMemset on d_initP5.", __LINE__);
		cudasafe(cudaMemcpy(d_initP5, initP, 4 * sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_initP5.", __LINE__);
	...
	case 7: // (x,y,bg,I,z,w)
		cudasafe(cudaMalloc((void**)& d_Parameters, (NV_PSP4D + 1)* Nfitraw * sizeof(float)), "Failed cudaMalloc on d_Parameters", __LINE__);
		cudasafe(cudaMemset(d_Parameters, 0, (NV_PSP4D + 1)* Nfitraw * sizeof(float)), "Failed cudaMemset on d_Parameters.", __LINE__);
		cudasafe(cudaMalloc((void**)& d_CRLBs, NV_PSP4D* Nfitraw * sizeof(float)), "Failed cudaMalloc on d_CRLBs.", __LINE__);
		cudasafe(cudaMemset(d_CRLBs, 0, NV_PSP4D* Nfitraw * sizeof(float)), "Failed cudaMemset on d_CRLBs.", __LINE__);
		plhs[0] = mxCreateNumericMatrix(Nfitraw, (NV_PSP4D + 1), mxSINGLE_CLASS, mxREAL);
		plhs[1] = mxCreateNumericMatrix(Nfitraw, NV_PSP4D, mxSINGLE_CLASS, mxREAL);
		break;

	case 8:// (x,y,bg,I,z,w,u)
		cudasafe(cudaMalloc((void**)& d_Parameters, (NV_P53 + 1)* Nfitraw * sizeof(float)), "Failed cudaMalloc on d_Parameters", __LINE__);
		cudasafe(cudaMemset(d_Parameters, 0, (NV_P53 + 1)* Nfitraw * sizeof(float)), "Failed cudaMemset on d_Parameters.", __LINE__);
		cudasafe(cudaMalloc((void**)& d_CRLBs, NV_P53* Nfitraw * sizeof(float)), "Failed cudaMalloc on d_CRLBs.", __LINE__);
		cudasafe(cudaMemset(d_CRLBs, 0, NV_P53* Nfitraw * sizeof(float)), "Failed cudaMemset on d_CRLBs.", __LINE__);
		plhs[0] = mxCreateNumericMatrix(Nfitraw, (NV_P53 + 1), mxSINGLE_CLASS, mxREAL);
		plhs[1] = mxCreateNumericMatrix(Nfitraw, NV_P53, mxSINGLE_CLASS, mxREAL);
		break;
	}
...
if  (silent==0)
   ...
   //             dim3 dimGrid(Nfitraw);
	...
	case 6:
		kernel_bsplineMLEFit_z_EMCCD_wrapper(dimGrid, dimBlock, d_data, d_coeff, spline_xsize - 3, spline_ysize - 3, spline_zsize - 3, (int)sz, iterations,
			d_Parameters, d_CRLBs, d_LogLikelihood, (const int)Nfitraw, d_Deg, temp);
		break;

	case 7:
		kernel_bsplineMLEFit_4D_EMCCD_wrapper(dimGrid, dimBlock, d_data, d_coeff, spline_xsize - 3, spline_ysize - 3, spline_zsize - 3, spline_wsize - 3, (int)sz, iterations, d_initP4,
			d_Parameters, d_CRLBs, d_LogLikelihood, (const int)Nfitraw, d_Deg );
		break;

	case 8:
		kernel_splineMLEFit_5n3D_EMCCD_wrapper(dimGrid, dimBlock, d_data, d_coeff, d_coeff3, spline_xsize - 3, spline_ysize - 3, spline_zsize - 3, spline_wsize - 3, spline_usize - 3, (int)sz, iterations, d_initP5,
			d_Parameters, d_CRLBs, d_LogLikelihood, (const int)Nfitraw, d_Deg);
		break;
	...
	case 6: // (x,y,bg,I,z)
		...
		break;
	case 7: // (x,y,bg,I,z,w)
		cudasafe(cudaMemcpy((void*)mxGetData(plhs[0]), d_Parameters, (NV_PSP4D + 1)* Nfitraw * sizeof(float), cudaMemcpyDeviceToHost),
			"cudaMemcpy failed for d_Parameters.", __LINE__);
		cudasafe(cudaMemcpy((void*)mxGetData(plhs[1]), d_CRLBs, NV_PSP4D* Nfitraw * sizeof(float), cudaMemcpyDeviceToHost),
			"cudaMemcpy failed for d_CRLBs.", __LINE__);
		break;

	case 8: // (x,y,bg,I,z,w)
		cudasafe(cudaMemcpy((void*)mxGetData(plhs[0]), d_Parameters, (NV_P53 + 1)* Nfitraw * sizeof(float), cudaMemcpyDeviceToHost),
			"cudaMemcpy failed for d_Parameters.", __LINE__);
		cudasafe(cudaMemcpy((void*)mxGetData(plhs[1]), d_CRLBs, NV_P53* Nfitraw * sizeof(float), cudaMemcpyDeviceToHost),
			"cudaMemcpy failed for d_CRLBs.", __LINE__);
		break;
	}
	...
	cudasafe(cudaFree(d_coeff3), "cudaFree failed on d_coeff3.", __LINE__);
	cudasafe(cudaFree(d_initP5), "cudaFree failed on d_initP5.", __LINE__);
