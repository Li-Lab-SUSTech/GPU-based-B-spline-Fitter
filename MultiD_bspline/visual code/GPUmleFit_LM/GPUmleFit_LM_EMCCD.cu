...
__global__ void kernel_splineMLEFit_z_EMCCD(const float *d_data,const float *d_coeff, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int sz, const int iterations, 
	 ...
	 */
			...
			/* newTheta[2]=max(0.0, (Nmax-newTheta[3])*2*pi*3*3);*/
...
__global__ void kernel_bsplineMLEFit_z_EMCCD(const float* d_data, const float* d_coeff, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int sz, const int iterations,
	...
	float newClamp[NV] = { 2,2,10000,20,20 }, oldClamp[NV] = { 2,2,10000,20,20 };
	float newDudt[NV] = { 0 };
  ...
  //initial values
	...
	/*newClamp[2]=max(newTheta[2],newClamp[2]);  //by fan
...
*/
	...
	xc = (sz + 1) / 2 - newTheta[1];
	yc = (sz + 1) / 2 - newTheta[0];
	zc = newTheta[4];
	off = (spline_xsize - sz) / 2.0f;
	xs = xc - floor(xc);
	ys = yc - floor(yc);
	zs = zc - floor(zc);
...
__global__ void kernel_bsplineMLEFit_4D_EMCCD(const float* d_data, const float* d_coeff, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int spline_wsize, const int sz, const int iterations, const float* d_initP4,
	float* d_Parameters, float* d_CRLBs, float* d_LogLikelihood, const int Nfits, float* deg) {

	//__shared__ float s_coeff[17*17*25];

	const int NV = NV_PSP4D;
	float M[NV * NV], Diag[NV], Minv[NV * NV];
	int tx = threadIdx.x;
	int bx = blockIdx.x;
	int BlockSize = blockDim.x;
	int ii, jj, kk, ll, l, m, i, conv = 0;
	...
	float Div;
	//float dudt[NV_PS];
	float newTheta[NV+1], oldTheta[NV];
	float newLambda = 1.0, oldLambda = 1.0;
	float newSign[NV] = { 0 }, oldSign[NV] = { 0 };
	float newUpdate[NV] = { 0 }, oldUpdate[NV] = { 0 };
	float newClamp[NV] = { 2.0,2.0,10000,20,20,0.001 }, oldClamp[NV] = { 2.0,2.0,10000,20,20,0.001 };
	float newDudt[NV] = { 0 };
  ...
  //initial values
	...
	{
		newTheta[4] = spline_zsize / 2.0f;
		newTheta[5] = spline_wsize / 2.0f;
	}

	//newTheta[4] = spline_zsize / 2.0f;
	//newTheta[5] = spline_wsize / 2.0f;

	//newClamp[3] = max(newTheta[3], newClamp[3]);  by fan
	...
	for (kk = 0; kk < iterations; kk++) {//main iterative loop
		...
		loop1:
			if (info == 0) {
				...
				//let it go
				if (((abs(newUpdate[4]) < 0.005) && turn) || (kk > 0.5 * iterations))
				{
					newClamp[5] = float(spline_wsize) / 5; oldClamp[5] = float(spline_wsize) / 5; turn = 0;
				...
				newTheta[2] = max(newTheta[2], 1.0);
				newTheta[3] = max(newTheta[3], 0.01);
				newTheta[4] = max(newTheta[4], 1.0);
				newTheta[4] = min(newTheta[4], float(spline_zsize - 0.5));
				newTheta[5] = max(newTheta[5], 1.0);
				newTheta[5] = min(newTheta[5], float(spline_wsize) - 0.5);

				//if (newTheta[5]<2 || newTheta[5]>float(spline_wsize) - 1) break;
	...
	//write to global arrays
	for (kk = 0; kk < NV+1; kk++) d_Parameters[Nfits * kk + BlockSize * bx + tx] = newTheta[kk];
	for (kk = 0; kk < NV; kk++) d_CRLBs[Nfits * kk + BlockSize * bx + tx] = Diag[kk];
	d_LogLikelihood[BlockSize * bx + tx] = Div;
	//d_LogLikelihood[BlockSize*bx+tx] = 1;
	//if (conv == 0)
	//	iter[bx * BlockSize + tx] = float(iterations);
...
__global__ void kernel_bsplineMLEFit_5n3D_EMCCD(const float* d_data, const float* d_coeff5, const float* d_coeff3, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int spline_wsize, const int spline_usize, const int sz, const int iterations, const float* initP5,
  ...
  //initial values
	...
	//newTheta[7] = float(0.5);
	for (i = 4; i < NV; i++)
	{
		newTheta[i] = *(initP + i - 4);
	}
	//for (ii = 0 ; ii < 5; ii++)
	//	newTheta[4 + i] = *(initP+ii);


	newClamp[4] = float(spline_zsize) / 10.0f;
	newClamp[5] = float(spline_wsize) / 10.0f;
	newClamp[6] = float(spline_usize) / 10.0f;
	...
	/*	if (kk >= 10)
		...
		loop1:
			if (info == 0) {
				...
				{
					newTheta[5] = newTheta[5] - spline_wsize;
				}
				//newTheta[5] = max(newTheta[5], 1);
				//newTheta[5] = min(newTheta[5], float(spline_wsize) - 0.5);

				newTheta[6] = max(newTheta[6], 1.0f);
				newTheta[6] = min(newTheta[6], float(spline_usize) - 1.0f);

				newTheta[7] = min(newTheta[7], 1.0f);// ratio
				newTheta[7] = max(newTheta[7], 0.0f);
