...
void mexFunction(int nlhs, mxArray *plhs[],	int	nrhs, const	mxArray	*prhs[]) {
	...
	*/
	...
	float *Parameters=0,*CRLBs=0,*LogLikelihood=0, * deg = 0;;
	...
	float *coeff=0,*d_coeff=0, * coeff3 = 0;
...
if  (silent==0)
{
	...
	case 6:
		...
		break;
	case 7:
		plhs[0] = mxCreateNumericMatrix(Nfitraw, NV_4D + 1, mxSINGLE_CLASS, mxREAL);
		plhs[1] = mxCreateNumericMatrix(Nfitraw, NV_4D, mxSINGLE_CLASS, mxREAL);
		break;
	case 8:
...
//#endif
		...
		case 6: //fit x,y,bg,I,z
			for (int ii = 0; ii < Nfitraw; ii++)
			kernel_bsplineMLEFit_z_EMCCD(ii, data, coeff, spline_xsize - 3, spline_ysize - 3, spline_zsize - 3, (int)sz, iterations,
				Parameters, CRLBs, LogLikelihood, Nfitraw, deg);
			break;

		case 7: //fit x,y,bg,I,z,w
			for (int ii = 0; ii < Nfitraw; ii++)
				kernel_bsplineMLEFit_z_EMCCD_4D(ii, data, coeff, spline_xsize - 3, spline_ysize - 3, spline_zsize - 3, spline_wsize - 3, (int)sz, iterations,initP,
					Parameters, CRLBs, LogLikelihood, Nfitraw, deg);
			break;

		case  8://fit x,y,bg,I,z,w,u
			for (int ii = 0; ii < Nfitraw; ii++)
				kernel_bsplineMLEFit_z_EMCCD_53D(ii, data, coeff, coeff3, spline_xsize - 3, spline_ysize - 3, spline_zsize - 3, spline_wsize - 3, spline_usize - 3, (int)sz, iterations, initP,
					Parameters, CRLBs, LogLikelihood, Nfitraw, deg);
			break;
